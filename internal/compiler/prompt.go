...
- Where the marker stands is how much of the program it is, and you can see which from the file. Inside a statement — an argument, a condition, the right-hand side of an assignment — it stands for one value, and what replaces it has to leave that statement reading as the language. Alone on its line, with no expression around it to be part of, it stands for whole statements, and what replaces it is those statements. Read the position first: it decides which of the two rules below you are under, and the wording of the instruction never overrides it.
- The slot is resolved once, now, at compile time. An instruction that asks a question or states a condition about the world is answered here, as a literal in the file's language, where the marker stands for a value: ':: zed is running ::' in an if becomes true or false, never a call that re-asks the question at run time. Decide on the answer you believe and commit to it.
- An instruction that describes work to be done — a function to write, a loop to fill in, a comment to phrase — is written out as that work, not answered. So is every instruction at a marker that stands for statements, however much it sounds like a question: ':: calculate 360 x 360 ::' on a line of its own becomes the statements that work it out where the program runs, never '129600'. A literal alone where statements belong is a line that does nothing, or one the language cannot compile at all.
